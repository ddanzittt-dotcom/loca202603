  ...
  const loadCloudData = useCallback(async (user) => {
    ...
    try {
      ...
      const nextProfile = {
        ...
        emoji: (profile.avatar_url && !profile.avatar_url.startsWith("http")) ? profile.avatar_url : me.emoji,
        avatarUrl: (profile.avatar_url && profile.avatar_url.startsWith("http")) ? profile.avatar_url : null,
