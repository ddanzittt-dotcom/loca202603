...
>
> **상단 배너 내비 통일 (2026-07-12)**:
> 데스크톱(≥960px) 1차 내비는 좌측 고정 레일에서 **상단 풀블리드 배너(58px, 전 탭 공통)** 로 통일했습니다. 탭 묶음은 뷰포트 정중앙, 로고는 왼쪽, 계정 버튼과 로카냥 듀오는 오른쪽. 위의 2026-06 "좌측 고정 워크스페이스 내비게이션" 서술은 과거 이력입니다. 상세 규격은 [0A.2](#0a2-desktop-shell).
>
...
- 1차 내비게이션은 `탐색`, `내 지도`, `내 장소`, `내 대시보드` 4탭입니다 (비로그인 시 `탐색`/`로그인`). 내 정보 관리는 우상단 계정 버튼으로 진입합니다.
- 지도 제작은 별도 탭이 아니라 `내 지도`의 "새 지도" 버튼과 지도 카드에서 진입합니다.
- `내 지도`는 지도 관리 허브입니다. 검색, 상태 필터, 협업 초대, 정렬, 편집 진입이 한 화면에서 보여야 합니다.
- `내 장소`는 기록 데이터베이스입니다. 검색, 필터, 유형별 탐색이 웹의 표/리스트 밀도로 보여야 합니다.
- `내 대시보드`는 등록·기록 데이터를 요약해 보여주는 수첩 화면입니다.

### 0A.2 Desktop Shell

- 960px 이상 1차 내비는 **상단 풀블리드 배너**를 전 탭 공통으로 사용합니다: 높이 58px, 배경 `#F6EFDB`, 하단 `2px solid #1F1A12` 보더 + `0 2px 0 rgba(31,26,18,0.12)` 그림자. (구 좌측 고정 레일 240px은 2026-07 제거)
- 배너 구성: 로고(왼쪽, 세로 구분선) → 탭 묶음(**뷰포트 정중앙 고정** — 로고 폭과 무관) → 계정 버튼(오른쪽 끝, 배너 안 세로 중앙) → 로카냥+치즈냥 듀오(계정 버튼 왼쪽, 배너에 걸터앉는 위 정렬).
- 콘텐츠는 배너 아래 풀폭 캔버스입니다. `.screen--scroll` 상단 패딩 94px(탐색만 82px 압축값), sticky 요소 top 기준은 72~78px.
...
- 지도 편집기는 데스크톱에서도 배너를 숨기고 전체 화면을 씁니다 (지도 조작 우선).

### 0A.3 Mobile Shell

- 959px 이하에서는 하단 탭 내비게이션을 유지합니다 (로그인 시 4탭).
